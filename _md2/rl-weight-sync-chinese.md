...
-   **易维护** - 轻量级代码库，并可从Megatron预训练平滑过渡到SGLang部署。[^1]([slime:)
...
![什么是权重同步？](https://cdn.jsdelivr.net/gh/hebiao064/hebiao064.github.io@_md2zhihu_hebiao064githubio_6fda0ed4/rl-weight-sync-chinese/2367ff35d5ecfcf2-what_is_weight_sync.png)
...
![权重同步在slime中如何工作？](https://cdn.jsdelivr.net/gh/hebiao064/hebiao064.github.io@_md2zhihu_hebiao064githubio_6fda0ed4/rl-weight-sync-chinese/10089a8ac82c884b-how_weight_sync_works.png)
...
![我们的优化之旅](https://cdn.jsdelivr.net/gh/hebiao064/hebiao064.github.io@_md2zhihu_hebiao064githubio_6fda0ed4/rl-weight-sync-chinese/f105c42b86e5d6e9-our_optimization_journey.png)

> **注意**：上图是根据这个Github Issue[^3]([slime)里的所有PR做完之后往回捋出来的，以便更容易理解逻辑，实际上，我们没有按照上图所示的改进顺序进行，因为实际工作场景中自然是按照从易到难实现，而不是根据物理传输过程中的顺序。
...
![传统方法 vs CUDA IPC](https://cdn.jsdelivr.net/gh/hebiao064/hebiao064.github.io@_md2zhihu_hebiao064githubio_6fda0ed4/rl-weight-sync-chinese/41c41ca79a90bb97-base64_memraid.png)

#### 利用CUDA IPC Handler同卡零拷贝传输

![CUDA IPC如何工作](https://cdn.jsdelivr.net/gh/hebiao064/hebiao064.github.io@_md2zhihu_hebiao064githubio_6fda0ed4/rl-weight-sync-chinese/f5b06abbe12f10e7-cuda_ipc_transfer_memraid.png)
...
### 4.1 优化Megatron Worker中Tensor聚合过程：
*从60秒到50秒*
...
<td>每个rank有部分Tensor → 收集所有部分以重构完整Tensor</td>
</tr>
<tr>
<td><strong>流水线并行 (PP)</strong></td>
<td><code>broadcast</code></td>
<td>源rank有完整层 → 分发到其他PP Rank</td>
</tr>
<tr>
<td><strong>专家并行 (EP)</strong></td>
<td><code>broadcast</code></td>
<td>源rank有完整专家 → 分发到其他专家组</td>
</tr>
</table>
...
### 4.3 合并多个Tensor成一个Tensor：减少CUDA IPC开销：
*从30秒到20秒*
...
![太多CUDA IPC操作](https://cdn.jsdelivr.net/gh/hebiao064/hebiao064.github.io@_md2zhihu_hebiao064githubio_6fda0ed4/rl-weight-sync-chinese/5477dc6e8a2fe729-before_flatten.png)
...
<th><strong>主要活动</strong></th>
</tr>
<tr>
<td><strong>IPC句柄打开</strong></td>
<td>22ms</td>
<td>54%</td>
<td>CUDA IPC句柄创建和内存映射</td>
</tr>
<tr>
<td><strong>加载权重</strong></td>
<td>8ms</td>
<td>19%</td>
<td>实际权重加载和Tensor重构</td>
</tr>
<tr>
<td><strong>IPC句柄关闭</strong></td>
<td>11ms</td>
<td>27%</td>
<td>CUDA IPC清理和资源释放</td>
</tr>
<tr>
<td><strong>总计</strong></td>
<td><strong>41ms</strong></td>
<td><strong>100%</strong></td>
<td>SGLang中完整的权重更新周期</td>
</tr>
</table>

**关键发现**：**81%的时间**花费在CUDA IPC操作（打开+关闭）上，而只有**19%**用于实际权重加载。这解释了为什么合并多个Tensor可以提供如此显著的改进。

![扁平化后](https://cdn.jsdelivr.net/gh/hebiao064/hebiao064.github.io@_md2zhihu_hebiao064githubio_6fda0ed4/rl-weight-sync-chinese/fca6a6a593ac761a-after_flatten.png)
...
<td><strong>100%</strong></td>
<td><strong>相比合并前减少了51%</strong></td>
</tr>
</table>
...
- An SGLang-Native Post-Training Framework for RL Scaling](https://lmsys.org/blog/2025-07-09-slime/) : [[slime:]([slime:)

- Issue #132: Weight Sync Optimization in Colocate Mode](https://github.com/THUDM/slime/issues/132) : [[slime]([slime)
