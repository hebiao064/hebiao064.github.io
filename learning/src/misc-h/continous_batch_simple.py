...
class Scheduler:
    ...
    def run_batch(self, batch: ScheduleBatch) -> GenerationBatchResult:
        print(
            f"Running batch with forward mode: {batch.forward_mode} and batch size: {len(batch.reqs)}"
        )
        ...
        return GenerationBatchResult(
            next_token_ids=torch.randint(0, 100, (len(batch.reqs),))
        )
    ...
    def process_batch_result(self, batch: ScheduleBatch, result: GenerationBatchResult):
        for req, next_token_id in zip(batch.reqs, result.next_token_ids):
            ...
            print(
                f"Request {req.rid} updated with origin input ids: {req.origin_input_ids} and output ids: {req.output_ids}"
            )
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        scheduler.add_request(
            Req(
                rid=i,
                origin_input_ids=torch.randint(
                    0, 100, (random.randint(1, 5),)
                ).tolist(),
